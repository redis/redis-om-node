import { IdStrategy, Schema, SchemaDefinition, SchemaOptions } from '$lib/schema';
import { Entity } from '$lib/entity/entity';
import { generateIndexHash } from "$lib/indexer";
import { DataStructure } from '../../../../dist';
...
describe("#generateIndexHash", () => {
  ...
  describe.each([
    ...
    ["that overrides the keyspace prefix", {
      schemaDef: EMPTY_SCHEMA_DEF,
      options: { prefix: 'test-prefix' } as SchemaOptions,
      expectedHash: "JjmGXw/Whrk1zgxDb1q5Kk76A/g="
    }],
    ...
    it("generates the expected hash", () => {
      const schema = new Schema<TestEntity>(TestEntity, data.schemaDef, data.options)
      const hash = generateIndexHash(schema)
      expect(hash).toBe(data.expectedHash);
    })
  })
});
