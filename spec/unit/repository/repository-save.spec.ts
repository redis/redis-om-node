...
const ENTITY_WITH_ID = {
  [EntityId]: 'foo',
  [EntityKeyName]: 'key:foo',
  aString: A_STRING,
  aNumber: A_NUMBER,
  aBoolean: true
}
...
describe('Repository', () => {
  describe('#save', () => {
    ...
    describe('to a Hash', () => {
      ...
      describe.each([
        [
          'when saving an entity without an entityId but with a provided id',
          'foo',
          ENTITY,
          'foo',
          'SimpleEntity:foo',
        ...
        [
          'when saving an entity with an entityId *and* a provided id',
          'bar',
          ENTITY_WITH_ID,
          'bar',
          'SimpleEntity:bar',
        ...
        [
          'when saving an empty entity without an entityId but with a provided id',
          'foo',
          EMPTY_ENTITY,
          'foo',
          'SimpleEntity:foo',
        ...
        [
          'when saving an empty entity with an entityId *and* a provided id',
          'bar',
          EMPTY_ENTITY_WITH_ID,
          'bar',
          'SimpleEntity:bar',
          true
        ]
        ...
        beforeEach(async () => {
          returnedEntity = await repository.save(id, entity)
        })
    ...
    describe('to JSON', () => {
      ...
      describe.each([
        ...
        if (isEmpty)
          it('save the empty data to the key', () =>
            expect(redis.json.set).toHaveBeenCalledWith(
              expect.stringMatching(keyNameRegex),
              '$',
