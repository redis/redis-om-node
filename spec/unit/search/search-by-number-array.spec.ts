...
describe('Search', () => {
  describe('#query', () => {
    ...
    const AN_EQUAL_QUERY = `(@someNumbers:[${A_NUMBER} ${A_NUMBER}])`
    const A_NEGATED_EQUAL_QUERY = `(-@someNumbers:[${A_NUMBER} ${A_NUMBER}])`
    const A_GT_QUERY = `(@someNumbers:[(${A_NUMBER} +inf])`
    const A_NEGATED_GT_QUERY = `(-@someNumbers:[(${A_NUMBER} +inf])`
    const A_GTE_QUERY = `(@someNumbers:[${A_NUMBER} +inf])`
    const A_NEGATED_GTE_QUERY = `(-@someNumbers:[${A_NUMBER} +inf])`
    const AN_LT_QUERY = `(@someNumbers:[-inf (${A_NUMBER}])`
    const A_NEGATED_LT_QUERY = `(-@someNumbers:[-inf (${A_NUMBER}])`
    const AN_LTE_QUERY = `(@someNumbers:[-inf ${A_NUMBER}])`
    const A_NEGATED_LTE_QUERY = `(-@someNumbers:[-inf ${A_NUMBER}])`
    const A_BETWEEN_QUERY = `(@someNumbers:[${ANOTHER_NUMBER} ${A_NUMBER}])`
    const A_NEGATED_BETWEEN_QUERY = `(-@someNumbers:[${ANOTHER_NUMBER} ${A_NUMBER}])`
    ...
    beforeEach(() => {
      ...
      where = search.where('someNumbers')
    })
    ...
    describe('when generating a query with a number', () => {
      ...
      it('generates a query with .is.not.greaterThan', () =>
        expectToBeNegatedGTQuery(where.is.not.greaterThan(A_NUMBER)))
      ...
      it('generates a query with .is.greaterThanOrEqualTo', () =>
        expectToBeGTEQuery(where.is.greaterThanOrEqualTo(A_NUMBER)))
      it('generates a query with .is.not.greaterThanOrEqualTo', () =>
        expectToBeNegatedGTEQuery(where.is.not.greaterThanOrEqualTo(A_NUMBER)))
      ...
      it('generates a query with .is.not.lessThanOrEqualTo', () =>
        expectToBeNegatedLTEQuery(where.is.not.lessThanOrEqualTo(A_NUMBER)))

      it('generates a query with .between', () => expectToBeBetweenQuery(where.between(ANOTHER_NUMBER, A_NUMBER)))
      it('generates a query with .not.between', () =>
        expectToBeNegatedBetweenQuery(where.not.between(ANOTHER_NUMBER, A_NUMBER)))
      it('generates a query with .is.between', () => expectToBeBetweenQuery(where.is.between(ANOTHER_NUMBER, A_NUMBER)))
      it('generates a query with .is.not.between', () =>
        expectToBeNegatedBetweenQuery(where.is.not.between(ANOTHER_NUMBER, A_NUMBER)))
    })
  })
})
