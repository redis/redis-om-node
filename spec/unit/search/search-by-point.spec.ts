    ...
    describe('when generating a query with a point', () => {
      describe('and in a circle', () => {
        ...
        it('generates a query with .longitude.latitude.radius.m', () =>
          expectToBeNegatedMetersQuery(
            where.is.not.inCircle(circle => circle.longitude(longitude).latitude(latitude).radius(42).m)
          ))
        ...
        it('generates a query with .point(lng, lat).radius.kilometer', () =>
          expectToBeKilometersQuery(
            where.is.inCircle(circle => circle.origin(longitude, latitude).radius(42).kilometer)
          ))
        it('generates a query with .longitude.latitude.radius.km', () =>
          expectToBeNegatedKilometersQuery(
            where.is.not.inCircle(circle => circle.longitude(longitude).latitude(latitude).radius(42).km)
          ))
        ...
        it('generates a query with .longitude.latitude.radius.ft', () =>
          expectToBeNegatedFeetQuery(
            where.is.not.inCircle(circle => circle.longitude(longitude).latitude(latitude).radius(42).ft)
          ))
        ...
        it('generates a query with .longitude.latitude.radius.mi', () =>
          expectToBeNegatedMilesQuery(
            where.is.not.inCircle(circle => circle.longitude(longitude).latitude(latitude).radius(42).mi)
          ))
      })
    })
  })
