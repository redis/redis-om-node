...
describe('Search', () => {
  describe('#query', () => {
    ...
    describe('when generating a query with a boolean for a hash', () => {
      ...
      const expectToBeNegatedFalseQuery: BooleanChecker = search =>
        expect(search.query).toBe(A_NEGATED_FALSE_HASH_QUERY)
    ...
    describe('when generating a query with a boolean for JSON', () => {
      ...
      const expectToBeNegatedFalseQuery: BooleanChecker = search =>
        expect(search.query).toBe(A_NEGATED_FALSE_JSON_QUERY)
