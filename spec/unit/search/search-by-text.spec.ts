...
describe('Search', () => {
  describe('#query', () => {
    ...
    const expectToBeNegatedExactTextQuery: StringChecker = search =>
      expect(search.query).toBe(A_NEGATED_EXACT_TEXT_QUERY)
    const expectToBeTextQueryWithFuzzyMatching: StringChecker = search =>
      expect(search.query).toBe(A_TEXT_QUERY_WITH_FUZZY_MATCHING)

    const expectToBeNumberTextQuery: StringChecker = search => expect(search.query).toBe(A_NUMBER_TEXT_QUERY)
    const expectToBeNegatedNumberTextQuery: StringChecker = search =>
      expect(search.query).toBe(A_NEGATED_NUMBER_TEXT_QUERY)
    const expectToBeNumberExactTextQuery: StringChecker = search => expect(search.query).toBe(A_NUMBER_EXACT_TEXT_QUERY)
    const expectToBeNegatedNumberExactTextQuery: StringChecker = search =>
      expect(search.query).toBe(A_NEGATED_NUMBER_EXACT_TEXT_QUERY)
    const expectToBeNumberTextQueryWithFuzzyMatching: StringChecker = search =>
      expect(search.query).toBe(A_NUMBER_TEXT_QUERY_WITH_FUZZY_MATCHING)

    const expectToBeBooleanTextQuery: StringChecker = search => expect(search.query).toBe(A_BOOLEAN_TEXT_QUERY)
    const expectToBeNegatedBooleanTextQuery: StringChecker = search =>
      expect(search.query).toBe(A_NEGATED_BOOLEAN_TEXT_QUERY)
    const expectToBeBooleanExactTextQuery: StringChecker = search =>
      expect(search.query).toBe(A_BOOLEAN_EXACT_TEXT_QUERY)
    const expectToBeNegatedBooleanExactTextQuery: StringChecker = search =>
      expect(search.query).toBe(A_NEGATED_BOOLEAN_EXACT_TEXT_QUERY)
    const expectToBeBooleanTextQueryWithFuzzyMatching: StringChecker = search =>
      expect(search.query).toBe(A_BOOLEAN_TEXT_QUERY_WITH_FUZZY_MATCHING)
    ...
    describe('when generating for a query with a string', () => {
      ...
      it('generates a query with .match with fuzzyMatching enabled', () =>
        expectToBeTextQueryWithFuzzyMatching(where.match(A_STRING, { fuzzyMatching: true })))
      ...
      it('generates a query with .matches with fuzzyMatching enabled', () =>
        expectToBeTextQueryWithFuzzyMatching(where.matches(A_STRING, { fuzzyMatching: true })))
      ...
      it('generates a query with .not.exact.match', () =>
        expectToBeNegatedExactTextQuery(where.not.exact.match(A_STRING)))
      it('generates a query with .exactly.matches', () => expectToBeExactTextQuery(where.exactly.matches(A_STRING)))
      it('generates a query with .does.exactly.match', () =>
        expectToBeExactTextQuery(where.does.exactly.match(A_STRING)))
      it('generates a query with .does.not.exactly.match', () =>
        expectToBeNegatedExactTextQuery(where.does.not.exactly.match(A_STRING)))

      it('generates a query with .matchExact', () => expectToBeExactTextQuery(where.matchExact(A_STRING)))
      it('generates a query with .not.matchExact', () =>
        expectToBeNegatedExactTextQuery(where.not.matchExact(A_STRING)))
      ...
      it('generates a query with .does.not.matchExactly', () =>
        expectToBeNegatedExactTextQuery(where.does.not.matchExactly(A_STRING)))
    })
    ...
    describe('when generating a query with a number as a string', () => {
      ...
      it('generates a query with .match with fuzzyMatching enabled', () =>
        expectToBeNumberTextQueryWithFuzzyMatching(where.match(A_NUMBER, { fuzzyMatching: true })))
      ...
      it('generates a query with .matches with fuzzyMatching enabled', () =>
        expectToBeNumberTextQueryWithFuzzyMatching(where.matches(A_NUMBER, { fuzzyMatching: true })))
      it('generates a query with .does.match', () => expectToBeNumberTextQuery(where.does.match(A_NUMBER)))
      it('generates a query with .does.not.match', () =>
        expectToBeNegatedNumberTextQuery(where.does.not.match(A_NUMBER)))

      it('generates a query with .exact.match', () => expectToBeNumberExactTextQuery(where.exact.match(A_NUMBER)))
      it('generates a query with .not.exact.match', () =>
        expectToBeNegatedNumberExactTextQuery(where.not.exact.match(A_NUMBER)))
      it('generates a query with .exactly.matches', () =>
        expectToBeNumberExactTextQuery(where.exactly.matches(A_NUMBER)))
      it('generates a query with .does.exactly.match', () =>
        expectToBeNumberExactTextQuery(where.does.exactly.match(A_NUMBER)))
      it('generates a query with .does.not.exactly.match', () =>
        expectToBeNegatedNumberExactTextQuery(where.does.not.exactly.match(A_NUMBER)))

      it('generates a query with .matchExact', () => expectToBeNumberExactTextQuery(where.matchExact(A_NUMBER)))
      it('generates a query with .not.matchExact', () =>
        expectToBeNegatedNumberExactTextQuery(where.not.matchExact(A_NUMBER)))
      it('generates a query with .matchesExactly', () => expectToBeNumberExactTextQuery(where.matchesExactly(A_NUMBER)))
      it('generates a query with .does.matchExactly', () =>
        expectToBeNumberExactTextQuery(where.does.matchExactly(A_NUMBER)))
      it('generates a query with .does.not.matchExactly', () =>
        expectToBeNegatedNumberExactTextQuery(where.does.not.matchExactly(A_NUMBER)))
    })
    ...
    describe('when generating a query with a boolean as a string', () => {
      ...
      it('generates a query with .match with fuzzyMatching enabled', () =>
        expectToBeBooleanTextQueryWithFuzzyMatching(where.match(true, { fuzzyMatching: true })))
      ...
      it('generates a query with .not.exact.match', () =>
        expectToBeNegatedBooleanExactTextQuery(where.not.exact.match(true)))
      it('generates a query with .exactly.matches', () => expectToBeBooleanExactTextQuery(where.exactly.matches(true)))
      it('generates a query with .does.exactly.match', () =>
        expectToBeBooleanExactTextQuery(where.does.exactly.match(true)))
      it('generates a query with .does.not.exactly.match', () =>
        expectToBeNegatedBooleanExactTextQuery(where.does.not.exactly.match(true)))

      it('generates a query with .matchExact', () => expectToBeBooleanExactTextQuery(where.matchExact(true)))
      it('generates a query with .not.matchExact', () =>
        expectToBeNegatedBooleanExactTextQuery(where.not.matchExact(true)))
      it('generates a query with .matchesExactly', () => expectToBeBooleanExactTextQuery(where.matchesExactly(true)))
      it('generates a query with .does.matchExactly', () =>
        expectToBeBooleanExactTextQuery(where.does.matchExactly(true)))
      it('generates a query with .does.not.matchExactly', () =>
        expectToBeNegatedBooleanExactTextQuery(where.does.not.matchExactly(true)))
    })
    ...
    describe('when trying to perform string equality full-text', () => {
      const EXPECTED_EXCEPTION =
        "Cannot call .equals on a field of type 'text', either use .match to perform full-text search or change the type to 'string' in the Schema."
