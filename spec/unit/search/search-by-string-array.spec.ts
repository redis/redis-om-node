...
describe('Search', () => {
  describe('#query', () => {
    ...
    const expectToBeNegatedContainsOneQuery: ArrayChecker = search =>
    ...
    describe('when generating for an array', () => {
      ...
      it('generates a query with .does.not.contain', () =>
        expectToBeNegatedContainsQuery(where.does.not.contain(A_STRING)))

      it('generates a query with .containsOneOf', () =>
        expectToBeContainsOneQuery(where.containsOneOf(A_STRING, ANOTHER_STRING, A_THIRD_STRING)))
      it('generates a query with .does.containOneOf', () =>
        expectToBeContainsOneQuery(where.does.containOneOf(A_STRING, ANOTHER_STRING, A_THIRD_STRING)))
      it('generates a query with .does.not.containOneOf', () =>
        expectToBeNegatedContainsOneQuery(where.does.not.containOneOf(A_STRING, ANOTHER_STRING, A_THIRD_STRING)))
      ...
      it('generates a query with .contains with a single underscore', () => {
        let query = where.contains('_').query
        expect(query).toBe('(@someStrings:{\\_})')
      })

      it('generates a query with .containsOneOf with a single underscore', () => {
        let query = where.containsOneOf('_', A_STRING).query
        expect(query).toBe(`(@someStrings:{\\_|${A_STRING}})`)
      })
      ...
      it('generates a query with .contains with a suffix matching wildcard', () => {
        let query = where.contains('*foo').query
        expect(query).toBe('(@someStrings:{*foo})')
      })

      it('generates a query with .containsOnOf with a suffix matching wildcard', () => {
        let query = where.containsOneOf('*foo').query
        expect(query).toBe('(@someStrings:{*foo})')
      })

      it('generates a query with .contains with a prefix and suffix matching wildcard', () => {
        let query = where.contains('*foo*').query
        expect(query).toBe('(@someStrings:{*foo*})')
      })

      it('generates a query with .containsOnOf with a prefix and suffix matching wildcard', () => {
        let query = where.containsOneOf('*foo*').query
        expect(query).toBe('(@someStrings:{*foo*})')
      })
    })
