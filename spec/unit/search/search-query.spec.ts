  ...
  describe('#query', () => {
    ...
    describe('when querying against hashes', () => {
      ...
      it('throws an exception when invoked with a missing field', () => {
        expect(() => search.where('missingString')).toThrowErrorOfType(
          FieldNotInSchema,
          "The field 'missingString' is not part of the schema and thus cannot be used to search."
        )
      })
      ...
      it('generates a query using .where', () => {
        let query = search
          ...
          .matches(A_STRING)
          .where('aPoint')
          .inCircle(circle => circle.origin(A_POINT).radius(POINT_RADIUS).miles)
          .where('aDate')
          .onOrAfter(A_DATE)
          .where('someStrings')
          .containsOneOf(A_STRING, ANOTHER_STRING).query
        expect(query).toBe(
          `( ( ( ( ( ( (${EXPECTED_STRING_QUERY_1}) (${EXPECTED_NUMBER_QUERY_1}) ) (${EXPECTED_TRUE_HASH_QUERY}) ) (${EXPECTED_TEXT_QUERY}) ) (${EXPECTED_POINT_QUERY}) ) (${EXPECTED_DATE_QUERY}) ) (${EXPECTED_ARRAY_QUERY}) )`
        )
      })
      ...
      it('generates a query using .and and .or', () => {
        let query = search.where('aString').eq(A_STRING).and('aNumber').equals(A_NUMBER).or('aBoolean').true().query
        expect(query).toBe(
          `( ( (${EXPECTED_STRING_QUERY_1}) (${EXPECTED_NUMBER_QUERY_1}) ) | (${EXPECTED_TRUE_HASH_QUERY}) )`
        )
      })
      ...
      it('generates a query using .and with a function', () => {
        let query = search
          ...
          .eq(A_STRING)
          .and(search =>
            search.where('aString').eq(ANOTHER_STRING).and('aNumber').equals(A_NUMBER).or('aBoolean').true()
          ).query
        expect(query).toBe(
          `( (${EXPECTED_STRING_QUERY_1}) ( ( (${EXPECTED_STRING_QUERY_2}) (${EXPECTED_NUMBER_QUERY_1}) ) | (${EXPECTED_TRUE_HASH_QUERY}) ) )`
        )
      })
      ...
      it('generates a query using .where with a function', () => {
        let query = search.where(search =>
          search.where('aString').eq(A_STRING).and('aNumber').equals(A_NUMBER).or('aBoolean').true()
        ).query
        expect(query).toBe(
          `( ( (${EXPECTED_STRING_QUERY_1}) (${EXPECTED_NUMBER_QUERY_1}) ) | (${EXPECTED_TRUE_HASH_QUERY}) )`
        )
      ...
      it('generates a complex query using all the things', () => {
        let query = search
          ...
          .onOrAfter(A_DATE)
          .and(search =>
            search.where('aString').eq(ANOTHER_STRING).and('aNumber').equals(ANOTHER_NUMBER).and('aBoolean').false()
          )
          .or(search =>
            search
              .where('aString')
              .eq(A_THIRD_STRING)
              .or('aNumber')
              .equals(A_THIRD_NUMBER)
              .or('aBoolean')
              .true()
              .and(search => search.where('aString').eq(A_STRING).and('aNumber').equals(A_NUMBER))
          ).query
    ...
    describe('when querying against JSON objects', () => {
      ...
      it('generates a query using .where', () => {
        let query = search
          ...
          .containsOneOf(A_STRING, ANOTHER_STRING).query
        expect(query).toBe(
          `( ( ( ( ( ( (${EXPECTED_STRING_QUERY_1}) (${EXPECTED_NUMBER_QUERY_1}) ) (${EXPECTED_TRUE_JSON_QUERY}) ) (${EXPECTED_TEXT_QUERY}) ) (${EXPECTED_POINT_QUERY}) ) (${EXPECTED_DATE_QUERY}) ) (${EXPECTED_ARRAY_QUERY}) )`
        )
      })
      ...
      it('generates a query using .and and .or', () => {
        let query = search.where('aString').eq(A_STRING).and('aNumber').equals(A_NUMBER).or('aBoolean').true().query
        expect(query).toBe(
          `( ( (${EXPECTED_STRING_QUERY_1}) (${EXPECTED_NUMBER_QUERY_1}) ) | (${EXPECTED_TRUE_JSON_QUERY}) )`
        )
      })
      ...
      it('generates a query using .and with a function', () => {
        ...
        expect(query).toBe(
          `( (${EXPECTED_STRING_QUERY_1}) ( ( (${EXPECTED_STRING_QUERY_2}) (${EXPECTED_NUMBER_QUERY_1}) ) | (${EXPECTED_TRUE_JSON_QUERY}) ) )`
        )
      })
