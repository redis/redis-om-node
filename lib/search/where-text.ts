...
export class WhereText<T extends Entity> extends WhereField<T> {
  ...
  eq(_: string | number | boolean): Search<T> {
    return this.throwEqualsExcpetion()
  }
  equal(_: string | number | boolean): Search<T> {
    return this.throwEqualsExcpetion()
  }
  equals(_: string | number | boolean): Search<T> {
    return this.throwEqualsExcpetion()
  }
  equalTo(_: string | number | boolean): Search<T> {
    return this.throwEqualsExcpetion()
  }

  toString(): string {
    const escapedValue = this.escapePunctuationForValue(this.value)

    if (this.exactValue) {
      return this.buildQuery(`"${escapedValue}"`)
    } else if (this.fuzzyMatching) {
      return this.buildQuery(
        `${'%'.repeat(this.levenshteinDistance)}${escapedValue}${'%'.repeat(this.levenshteinDistance)}`
      )
    } else {
      return this.buildQuery(`'${escapedValue}'`)
    }
